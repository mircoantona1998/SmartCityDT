...
const devices = JSON.parse(fs.readFileSync('lamps.json', 'utf8'));
...
function startIndependentLamp(deviceConfig) {
  ...
  client.open((err) => {
    if (err) {
      ...
      setInterval(() => {
        const stato = Math.random() < 0.5 ? 1 : 2;
        const OperatingHours = (Math.random() * 24).toFixed(2);
        
        const data = JSON.stringify({
          stato_lampione: stato,
...
devices.forEach(startIndependentLamp);
